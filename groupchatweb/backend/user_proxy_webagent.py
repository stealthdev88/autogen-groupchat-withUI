...
class UserProxyWebAgent(autogen.UserProxyAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.original_messages_length = 0
    ...
    async def a_check_termination_and_human_reply(
        ...
        if self.human_input_mode == "ALWAYS":
            self.new_reply_messages = messages[self.original_messages_length:]
            self.original_messages_length = len(messages)
    ...
    async def a_get_human_input(self, prompt: str) -> str:
        ...
        if last_message["content"]:
            for i in range(1, len(self.new_reply_messages)):
                message = self.new_reply_messages[i]
                await self.client_receive_queue.put(message["content"])
